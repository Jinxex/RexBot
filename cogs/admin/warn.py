...
class WarnSystem(commands.Cog):
    ...
    async def warn(self, ctx, member: discord.Member, reason=None):
        ...
        cursor.execute(
            "INSERT INTO warnings (member_id, reason) VALUES (?, ?)",
            (member.id, reason),
        )
        self.connection.commit()

        embed = discord.Embed(
            title=f"{member.mention} was warned.", description=f"Reason: **{reason}**"
        )
    ...
    async def warnings(self, ctx, member: discord.Member):
        self.cursor.execute(
            "SELECT reason FROM warnings WHERE member_id = ?", (member.id,)
        )
        result = self.cursor.fetchall()

        if result:
            embed = discord.Embed(
                title=f"Warnings for {member.name}", color=discord.Color.red()
            )
            ...
            embed2 = discord.Embed(
                title=f"There are no warnings for {member.name}",
    ...
    async def leaderboard(self, ctx):
        self.cursor.execute(
            "SELECT member_id, COUNT(*) FROM warnings GROUP BY member_id ORDER BY COUNT(*) DESC"
        )
        result = self.cursor.fetchall()

        if result:
            embed = discord.Embed(
                title="Warning Leaderboard", color=discord.Color.gold()
            )
            for idx, row in enumerate(result):
                member = ctx.guild.get_member(row[0])
                warnings_count = row[1]
                embed.add_field(
                    name=f"Place {idx + 1}: {member.name}",
                    value=f"Number of warnings: {warnings_count}",
                    inline=False,
                )
            await ctx.respond(embed=embed)
        else:
