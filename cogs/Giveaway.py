...
class Giveaway(commands.Cog):
    ...
    async def giveaway_start(self, ctx,
                       time: Option(int, "Wie lange soll das Giveaway dauern? (in Minuten)", min_value=1, max_value=1440)):
...
class Modal(discord.ui.Modal):
    ...
    async def callback(self, interaction):
        ...
        embed.add_field(name="Teilnehmen", value="Klicke auf den Button unten, um am Giveaway teilzunehmen.")
        embed.add_field(name="Endet in", value=ezcord.times.dc_timestamp(time, "R"), inline=False)
        ...
        async with aiosqlite.connect("giveaway.db") as db:
            async with db.execute(
                ...
                for row in trow:
                    teilnehmer += 1
                if teilnehmer < 2:
                    embed = discord.Embed(title="Giveaway abgebrochen", description="Niemand hat am Giveaway teilgenommen.",
                                            color=discord.Color.red())
                    n = datetime.datetime.now()
                    embed.add_field(name="Giveaway Informationen",
                    ...
                    for row in grow:
                        ...
                        if gewinner == teilnehmer2:
                            ...
                            n = datetime.datetime.now()
                            embed.add_field(name="Giveaway Informationen",value=f"Veranstaltet von {interaction.user.mention}\nGestartet um {discord.utils.format_dt(b, 'R')}\nBeendet um {discord.utils.format_dt(n, 'R')}\nDauer des Giveaways: {time / 60} Minuten\nTeilnehmer: {teilnehmer}")
                            embed.add_field(name="Dauer", value=f"{time / 60} Minuten")
                            await interaction.channel.send(embed=embed)
                            embed2 = discord.Embed(title="Giveaway beendet", description="Siehe unten!",
...
class GvwButton(discord.ui.View):
    ...
    @discord.ui.button(label="Am Giveaway teilnehmen", style=discord.ButtonStyle.green, custom_id="join_giveaway")
    async def button_callback(self, button, interaction):
        async with aiosqlite.connect("giveaway.db") as db:
            async with db.execute("SELECT user_id FROM giveaway WHERE giveaway_id = ? AND user_id = ?",
                ...
                print(ids)
        if ids:
            await interaction.response.send_message("Du nimmst bereits am Giveaway teil. Möchtest du es verlassen?",
...
class LeaveButton(discord.ui.View):
    ...
    @discord.ui.button(label="Giveaway verlassen", style=discord.ButtonStyle.red, custom_id="leave")
    async def button_callback(self, button, interaction):
        await interaction.response.send_message("Du hast das Giveaway verlassen.", ephemeral=True)
        async with aiosqlite.connect("giveaway.db") as db:
            await db.execute("""
