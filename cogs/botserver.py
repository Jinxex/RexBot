    ...
    async def server_bot(self, ctx):
        if ctx.guild:
            ...
            if ctx.author.permissions.administrator:
                ...
                embed.add_field(name="Mitgliederzahl", value=server.member_count, inline=False)
                embed.add_field(name="Kanalanzahl", value=len(server.channels), inline=False)

                await ctx.respond(embed=embed, ephemeral=True)
            else:
                await ctx.respond("Du hast keine Berechtigung, diesen Befehl zu verwenden.")
        else:
