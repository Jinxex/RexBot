...
class Global(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        with self.db_connection as connection:
            ...
            cursor.execute('SELECT * FROM user_message_counts WHERE server_id = ? AND user_id = ?', (server_id, str(message.author.id)))
            user_data = cursor.fetchone()

        if server_data:
            anti_link_channel_id, spam_channel_id, sync_channel_id, spam_limit, spam_cooldown = server_data[1:6]

            if re.search("(?P<url>https://[^\s]+)", message.content):
                if message.channel.id == int(anti_link_channel_id):
                    await message.delete()
                    await message.author.send(":warning:**Daher der Dynamo Chat Aktiv ist auf diesem Server können keine Links gesendet werden.**")
                    return

            if message.channel.id == int(spam_channel_id):
                if user_data and user_data[2] >= spam_limit:
                    await message.delete()
                    await message.author.send(f":warning:**Du hast das Nachrichtenlimit erreicht. Bitte warte {spam_cooldown} Sekunden, bevor du es erneut sendest.**")
                    return

                if user_data:
                    cursor.execute('UPDATE user_message_counts SET message_count = ? WHERE server_id = ? AND user_id = ?', (user_data[2] + 1, server_id, str(message.author.id)))
                else:
                    cursor.execute('INSERT INTO user_message_counts (server_id, user_id, message_count) VALUES (?, ?, ?)', (server_id, str(message.author.id), 1))
    ...
    async def setup_server(self, ctx, anti_link_channel: discord.TextChannel, spam_channel: discord.TextChannel, sync_channel: discord.TextChannel, spam_limit: int, spam_cooldown: int):
        ...
        with self.db_connection as connection:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (server_id, anti_link_channel_id, spam_channel_id, sync_channel_id, spam_limit, spam_cooldown))
            connection.commit()

        await ctx.respond(f'Server erfolgreich eingerichtet!\nAnti-Link-Channel: {anti_link_channel.mention}\nSpam-Channel: {spam_channel.mention}\nSync-Channel: {sync_channel.mention}\nSpam-Limit: {spam_limit}\nSpam-Cooldown: {spam_cooldown}', ephemeral=True)
