...
class PunishView(discord.ui.View):
    ...
    async def ban(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        if interaction.user.top_role.position < self.user.top_role.position:
            await interaction.followup.send("Du kannst keinen Nutzer bannen, der eine höhere Rolle hat als du!",
                                            ephemeral=True)
            await asyncio.sleep(5)
            return
    ...
    @discord.ui.button(label="Kick", style=discord.ButtonStyle.blurple, custom_id="kick",
    ...
    async def kick(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        if self.user.guild_permissions.kick_members:
            await interaction.response.send_message("Du kannst keine Moderatoren kicken.", ephemeral=True)
            await asyncio.sleep(5)
        ...
        if interaction.user.top_role.position < self.user.top_role.position:
            await interaction.response.send_message("Du kannst keinen Nutzer kicken, der eine höhere Rolle hat als du!",
                                                    ephemeral=True)
            await asyncio.sleep(5)
            await interaction.delete_original_response()
            return
        ...
        if interaction.user.guild_permissions.kick_members:
            ...
            await interaction.response.send_message(f"{self.user.mention} wurde gekickt für `{self.reason}`",
                                                    ephemeral=True)
            await asyncio.sleep(5)
            await interaction.delete_original_response()
