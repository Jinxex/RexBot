...
class BirthdayDB(cloudcord.DBHandler):
    ...
    async def get_birthday_channel(self, guild_id):
        result = await self.fetchrow('SELECT channel_id FROM birthdays WHERE guild_id = ?', (guild_id,))
        return result['channel_id'] if result else None

    async def check_birthdays(self, today):
        return await self.fetchall('SELECT user_id FROM birthdays WHERE substr(birth_date, 6) = ?', (today,))
...
class BirthdayCog(cloudcord.Cog, emoji="🎂"):
    birthday = SlashCommandGroup("birthday", description="Schreiben Sie dort Ihren Geburtstag!🎂")
    ...
    @birthday.command(description="Geben Sie den Kanal ein, in den der Bot schreiben soll, wenn jemand Geburtstag hat!")
    ...
    async def callback(self, interaction):
        try:
            ...
            if not (1 <= day <= 31 and 1 <= month <= 12):
                await interaction.response.send_message("Ungültiges Datumsformat. Verwende das Format MM-DD.")
        ...
        await interaction.guild.me.edit(birthdays=interaction.user.id, birth_date=birth_date)
...
class TutorialView(discord.ui.View):
    ...
    async def button_callback(self, button, interaction):
        await interaction.response.send_modal(TutorialModal(title="Erstelle Dein Geburtstag"))
